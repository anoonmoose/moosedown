...
void BuildDB()
{
	...
	for (int i = cnt; i >= 1; i--)//get every page
	{
		var dx = GetPage(i);
		var tStart = DateTime.Now;//time after downloading page
		Listens.InsertAllOnSubmit(dx);
		dx.Last().Dump();
		if (i % 100 == 0) { SubmitChanges(); }//submit every 100 pages
...
void UpdateDB()
{
	...
	var dx = new List<Listen>();

	for (int i = 1; i <= cnt && !kl; i++)//keep going until we hit the end or last timestamp
	{
		dx.AddRange(GetPage(i).Where(x => x.Utime > lTime));//every track later than our last timestamp
	...
	Listens.InsertAllOnSubmit(dx.OrderBy(x=>x.Utime));
	SubmitChanges();
...
List<Listen> GetPage(int i)
{
	...
	return dt.OrderBy(x=>x.Utime).ToList();
}
...
{
	var qx = new Listen();
	if (t.Elements("date").Count() == 0)
	{
		qx.Utime = DateTimeOffset.Now.ToUnixTimeSeconds();
	}
	else
	{
